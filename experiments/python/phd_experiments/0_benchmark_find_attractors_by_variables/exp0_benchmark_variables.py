...
print(f"Iniciando Benchmark: {N_VAR_MIN} a {N_VAR_MAX} variables | Redes: {N_LOCAL_NETWORKS}")
print("-" * 60)
...
with open(csv_file, "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow([
        "sample_id", "n_variables", "n_local_networks", 
        "attractors_bf", "attractors_sat", "match", 
        "time_bf", "time_sat"
    ])
...
for n_var in range(N_VAR_MIN, N_VAR_MAX + 1):
    ...
    for i in range(1, N_SAMPLES + 1):
        ...
        with open(csv_file, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([i, n_var, N_LOCAL_NETWORKS, count_bf, count_sat, is_match, time_bf, time_sat])
