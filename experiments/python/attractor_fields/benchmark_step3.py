...
def run_benchmark_step3():
    ...
    for top_id in topologies:
        ...
        print(
            f"{top_name:<20} | {'Parallel (4 CPUs)':<30} | {par_time:>9.3f}s | {par_fields}"
        )
        ...
        print(
            f"{top_name:<20} | {'Parallel Chunks (4 CPUs)':<30} | {par_chunks_time:>9.3f}s | {par_chunks_fields}"
        )
        ...
        print(
            f"{top_name:<20} | {'Turbo (Numba)':<30} | {turbo_time:>9.3f}s | {turbo_fields}"
        )
        ...
        if not (seq_fields == par_fields == par_chunks_fields == turbo_fields):
            ...
            print(
                f"  Seq={seq_fields}, Par={par_fields}, ParChunks={par_chunks_fields}, Turbo={turbo_fields}"
            )
        else:
