    ...
    for net in o_cbn.l_local_networks:
        ...
        for scene in net.local_scenes:
            ...
            for attr in scene.l_attractors:
                ...
                states = tuple(
                    tuple(int(v) for v in s.l_variable_values[:n_internal])
                    for s in attr.l_states
                )
                if len(states) > 1:
                    rotations = [
                        tuple(states[i:] + states[:i]) for i in range(len(states))
                    ]
...
def verify_consistency_dynamic(o_cbn_sat, o_cbn_bf):
    ...
    if not mismatches:
        logging.info(
            "  [SUCCESS] All local networks yielded identical attractor signatures in all scenes."
        )
        return True
    else:
...
def main():
    ...
    logging.info(
        f"Generating Aleatory CBN with {N_LOCAL_NETWORKS} networks (Complete Topology)..."
    )
