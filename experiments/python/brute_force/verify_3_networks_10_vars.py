    ...
    for net in sorted(networks, key=lambda x: x.index):
        ...
        if out_signals:
            ...
            for s in out_signals:
                print(
                    f"      Signal {s.index_variable} (ID {s.index}): {s.coupling_function} using {s.l_output_variables}"
                )
        ...
        for scene_obj in net.local_scenes:
            ...
            for i, attr in enumerate(scene_obj.l_attractors):
                states = [
                    "".join(
                        str(v)
                        for v in s.l_variable_values[: len(net.internal_variables)]
                    )
                    for s in attr.l_states
                ]
                print(
                    f"        Attr {i+1:2} (len={len(states)}): {states if len(states) <= 2 else states[:2] + ['...']}"
                )
...
if bf_sum == sat_sum:
    print(
        " VERIFICATION SUCCESS: Both methods are 100% consistent for 3-nets, 30 variables!"
    )
else:
