...
if HAS_NUMBA:
    ...
    @njit(parallel=True)
    def evaluate_all_states_kernel(
        num_vars, cnf_data, clause_lengths, literal_lengths, external_values
    ):
        """
        ...
        for j, clause in enumerate(var.cnf_function):
            ...
            for k, lit in enumerate(clause):
                ...
                if local_idx is None:
                    logger.error(
                        f"Variable {global_idx} not found in network {local_network.index}"
                    )
                    continue
...
def find_attractors_turbo(local_network, scene_obj):
    ...
    next_map = evaluate_all_states_kernel(
        num_vars, cnf_data, clause_lengths, literal_lengths, scene_values
    )
